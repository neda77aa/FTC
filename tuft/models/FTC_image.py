...
class FTC(nn.Module):
    def __init__(self, AE, Transformer, frames_per_video, embedding_dim, rm_branch=None, num_hidden_layers=None,pretrained = False):
        ...
        self.aorticstenosispred = nn.Sequential(
            ...
            nn.LayerNorm(embedding_dim//2),
            nn.LeakyReLU(negative_slope=0.05, inplace=True),
            nn.Linear(in_features=embedding_dim//2, out_features=3, bias=True),
            # nn.Softmax(dim=2),
            #Reduce(),
            )
        
        self.viewpred = nn.Sequential(
            nn.Linear(in_features=embedding_dim, out_features=embedding_dim//2, bias=True),
            nn.LayerNorm(embedding_dim//2),
            nn.LeakyReLU(negative_slope=0.05, inplace=True),
            nn.Linear(in_features=embedding_dim//2, out_features=3, bias=True),
    ...
    def forward(self, x):   
        ...
        outputs = self.T(embeddings.view(-1, 1, self.em), output_hidden_states=True)
        ...
        view_prediction = self.viewpred(outputs)
        
#         # View Analysis
#         view_prob = torch.nn.functional.softmax(view_prediction,dim=2)
...
#         as_prediction = (as_prediction * att_weight).sum(1)
        ...
        return  view_prediction.squeeze(),as_prediction.squeeze()

def get_model_image(emb_dim, img_per_video, 
