...
def main():
    ...
    if True:
        pt_file =Path('/AS_Neda/AS_thesis/logs/true-puddle-20/best_model.pth')
    ...
    dataloader = get_as_dataloader(config, split='val', mode='val')
    ...
    features = np.array(feature[0].squeeze())
    labels_tot = np.array(labels_AS)
    for cur_iter, (inputs, labels_AS,label_B) in enumerate(dataloader):
        feature = feature_extractor(inputs)
        features = np.concatenate((features,feature[0].squeeze().numpy()))
